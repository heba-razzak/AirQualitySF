...
non_numeric_cols <- setdiff(names(wildfires_df), c(numeric_cols, "fire_id", "DECADES", "OBJECTIVE", "STATE", "INC_NUM", "FIRE_NAME"))
...
```{r}
wildfires_dates_seq <- wildfires_df %>% 
  mutate(ALARM_DATE = ymd(ALARM_DATE), CONT_DATE = ymd(CONT_DATE)) %>% 
  select(fire_id, ALARM_DATE, CONT_DATE, GIS_ACRES) %>%
  rowwise() %>% 
  mutate(fire_dates = list(full_seq(c(ALARM_DATE, CONT_DATE), 1)),
         fire_day = list(seq_along(full_seq(c(ALARM_DATE, CONT_DATE), 1)))
  ) %>%
  unnest(cols = c(fire_dates, fire_day)) %>%
  select(fire_id, fire_dates, fire_day)

wildfires_purpleair_df <- wildfires_purpleair_df %>% left_join(wildfires_dates_seq, by = c("fire_id"))
head(wildfires_dates_seq)
```
...
wind <- weather_pa %>% left_join(weather_data, by = "station") %>% select(sensor_index, timestamp, wind_direction, wind_speed)
wind
```

Closest fire: distance, direction, size, days since start
```{r}
pafire <- pafires %>%
  group_by(sensor_index, pa_date) %>%
  mutate(
    f_dist = ifelse(all(is.na(fire_distance)), 999999, min(fire_distance, na.rm = TRUE)),
    f_dir = ifelse(all(is.na(fire_distance)), -1, fire_direction[which.min(fire_distance)]),
    f_size = ifelse(all(is.na(fire_distance)), -1, GIS_ACRES[which.min(fire_distance)]),
    f_day = ifelse(all(is.na(fire_distance)), -9999, fire_day[which.min(fire_distance)]),
    fire_id = ifelse(all(is.na(fire_distance)), -9999, fire_id[which.min(fire_distance)])
  ) %>%
  select(-fire_distance, -fire_direction, -GIS_ACRES, -fire_day) %>%
  ungroup() %>% distinct()
