...
## Remove sensor 20349 (code commented below, has too many 0 readings)
```{r, remove-sensor}
purpleair_data <- purpleair_data %>%
  filter(sensor_index != 20349)
```

# remove sensors with too little readings (less than 10% of time period)
...
keep_sensors <- sensor_counts %>% filter(p > 0.1) %>% pull(sensor_index)
...
purpleair_processed_dates <- purpleair_complete_temp %>%
  ...
  ungroup() %>%
  select(-pm2.5_alt_a, -pm2.5_alt_b, -local_timestamp, -local_date, -location_type_mode)
```
...
# Add lagged columns for pm2.5_alt at 1, 3, 6, 12, and 24-hour intervals, grouped by sensor_index
purpleair_processed_lags <- purpleair_processed[
  order(sensor_index, time_stamp),  # Ensure correct order for lag calculation
  `:=`(
    pm2.5lag1 = data.table::shift(pm2.5_alt, n = 1, type = "lag"),
    pm2.5lag3 = data.table::shift(pm2.5_alt, n = 3, type = "lag"),
    pm2.5lag6 = data.table::shift(pm2.5_alt, n = 6, type = "lag"),
    pm2.5lag12 = data.table::shift(pm2.5_alt, n = 12, type = "lag"),
    pm2.5lag24 = data.table::shift(pm2.5_alt, n = 24, type = "lag")), by = sensor_index  # Apply within each sensor group
]

# use largest lag to remove all rows with missing lags
purpleair_final <- purpleair_processed_lags[!is.na(pm2.5lag24)]
```

```{r}
# Step 4: Save the processed data
write.csv(purpleair_final, file = file.path("data", "processed", "purpleair_data.csv"), row.names = FALSE)
```
...
purpleair_data <- purpleair_data %>%
  mutate(dow = factor(dow, levels = 1:7, labels = dow_labels))

# Group data and calculate mean PM2.5
heatmap_data <- purpleair_data %>%
  group_by(hour, dow) %>%
  summarise(mean_pm25 = mean(pm2.5_alt, na.rm = TRUE))

# Add AM/PM labels for the hour
heatmap_data <- heatmap_data %>%
  mutate(hour_label = case_when(
    hour == 0 ~ "12 AM",
...
# Order the hours in AM/PM format for the x-axis
heatmap_data$hour_label <- factor(heatmap_data$hour_label, levels = c(
  "12 AM", paste0(1:11, " AM"), "12 PM", paste0(1:11, " PM")
))

# Create heatmap
ggplot(heatmap_data, aes(x = hour_label, y = dow, fill = mean_pm25)) +
  geom_tile(color = "white") +  # Add gridlines for better readability
  scale_fill_gradient(low = "blue", high = "red", name = "Avg PM2.5") +  # Rename legend
  labs(
    title = "PM2.5 by Hour and Day of Week",
    x = "Hour",
    y = "Day of Week"
  ) +
  theme_minimal() +
...
time_features <- purpleair_data %>%
  mutate(is_rush_hour = as.numeric(hour %in% c(7,8,9,16,17,18)),
  ...
  select(time_stamp, dow, hour, holiday, is_rush_hour, is_weekend, season) 

# View as a table
print(time_features %>% sample_n(3))
```

```{r}
print(rbind(time_features %>% filter(holiday==0) %>% sample_n(1),
      time_features %>% filter(holiday==1) %>% sample_n(1),
      time_features %>% filter(holiday==0) %>% sample_n(1)
      ))
# print()
...
select sensor index
```{r}
purpleair_data %>% group_by(sensor_index) %>% summarize(n=n()) %>% arrange(desc(n)) %>% head(10) %>% pull(sensor_index)
```
...
```{r}
# 2883 3894 4299 3082 5151 2916 4478 2858 5650 3318
# Select a sensor to analyze
sensor_index <- 4478

# Get the PM2.5 data for the selected sensor
sensor_data <- purpleair_data %>% 
  filter(sensor_index == sensor_index & !is.na(pm2.5_alt))
...
```{r}
# List of sensor indices to analyze
sensor_indices <- c(2883, 3894, 4299, 3082, 5151, 2916, 4478, 2858, 5650, 3318)
...
for (sensor_index in sensor_indices) {
  ...
  # Check if there is enough data for the sensor
  if (nrow(sensor_data) > 0) {
    # Calculate the autocorrelation
    autocorr <- calc_autocorrelation(sensor_data)
    
    # Restrict to the first 25 lags
    autocorr_subset <- autocorr[1:25]
    
    # Plot the restricted autocorrelation
    plot(0:24, autocorr_subset, type = "b", 
         main = paste("Autocorrelation for Sensor", sensor_index),
         xlab = "Lag", ylab = "Autocorrelation")
  } else {
    cat("No data available for sensor", sensor_index, "\n")
  }
}
```
