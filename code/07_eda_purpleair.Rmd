...
filename <- paste0("purpleair_2018-01-01_2019-12-31.csv")
filepath <- file.path("data", "raw", filename)

# Read purple air data
purpleair_data <- data.frame(fread(filepath))
...
if (!file.exists(img_path)) {
  ...
  purpleair_data$missing <- ifelse(purpleair_data$pm2.5_alt_a == -1 | purpleair_data$pm2.5_alt_b == -1, "Missing", "Valid")
  
  p <- ggplot(purpleair_data, aes(x = pm2.5_alt_a, y = pm2.5_alt_b, color = missing)) +
    geom_point() +
