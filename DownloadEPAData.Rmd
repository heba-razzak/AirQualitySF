...
aqs_creds <- readr::read_file("inputs/aqs_credentials.txt")
...
```{r, download-aqs-data, eval=FALSE}
...
for (i in 1:nrow(monitor_info)) {
  ...
  write.csv(processed_data,
            file = paste0(preprocessing_directory, "/AQS/aqs_2019_", processed_data$id[i], ".csv"),
            row.names = FALSE)
}
```

```{r, combine-aqs-files, eval=FALSE}
# Get the list of AQS files
csv_files <- list.files(path = paste0(preprocessing_directory, "/AQS"), 
...
write.csv(combined_data, 
          file = paste0(preprocessing_directory, "/EPA_airquality.csv"), 
          row.names = FALSE)
```
...
plot_ly(epa_data_filtered, x = ~timestamp, y = ~pm25, color = ~id, type = 'scatter', mode = 'lines', connectgaps = FALSE) %>%
  layout(title = "PM2.5 Levels Over Time by ID",
